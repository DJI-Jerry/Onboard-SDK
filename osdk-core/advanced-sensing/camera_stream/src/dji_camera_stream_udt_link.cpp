...
DJICameraStreamUdtLink::DJICameraStreamUdtLink(CameraType c) : DJICameraStreamLink(c),
    ip(std::string(UDT_SERVER_IP)),
    fHandle(-1)
{
}

DJICameraStreamUdtLink::~DJICameraStreamUdtLink()
{
  cleanup();
}

bool DJICameraStreamUdtLink::init()
{
  UDT::startup();
...
void DJICameraStreamUdtLink::unInit()
{
  if(-1 !=fHandle)
  {
    UDT::close(fHandle);
...
void DJICameraStreamUdtLink::readThreadFunc()
{
